---
title: "Modeling of B-cell subset cluster frequencies w/ day 28 after 2nd vaccine antibody endpoint"
output: 
  html_document:
    code_folding: hide
editor_options: 
...
Plots for Fig. 4 and Fig. S3 in Kardava et al., PNAS 2022

Toggle between freq.type = "antigen-nonspecific" or "spike-specific" to run models using cluster size (relative to the number of CD19+ cells; Fig. S3 except S3E) 
or RBD+S1+ cell frequency (relative to number of cells in clusters; Fig. 4 and S3E), respectively
...
data.folder <- "Data"
output.folder <- paste0("final_output.v2d28.",norm.factor,".",gsub("_",".",variable.of.interest,fixed = T))
...
# antibody data
ab.4plex <- readRDS(file.path(data.folder,"pnas.4plex.results.early.endpoints.rds"))
...
#### Cluster Selection
Select clusters for further consideration based on cluster size and vaccine response
```{r cluster.selection}
...
```{r antibody}
ab.4plex <- merge(unique(freq.data[,-c(2,4)]),ab.4plex,all=T,by.x=c("subject_id","variable"),by.y=c("VAC.ID","Timepoint"))

# determine whether dose response relative to dose baseline are associated with antibody level at Day 28
...
for (term in c("d1.norm.freq","d2.norm.freq")) {
  ...
  for (day in days[[dose]]) {
    ...
    for (ab in unique(subset(ab.4plex,!is.na(Readout_AU.mL))$Readout_AU.mL)) {
      ab.single.D2D28 <- subset(ab.4plex,Readout_AU.mL == ab & dose.timepoint == "2.D28")
      ab.single.D2D28 <- cbind(ab.single.D2D28,dose.pop.freq[ab.single.D2D28$subject_id,,drop=F])
      ab.single.D2D28 <- cbind(ab.single.D2D28,metadata[ab.single.D2D28$subject_id,c("Age.at.day.of.vaccine","Gender.at.Birth")])
      for (i in c("S2P","RBD")) {
        # convert endpoint values to numeric
        ab.single.D2D28$endpoint <- ab.single.D2D28[,i]
        ab.single.D2D28$endpoint[grep("less than",ab.single.D2D28$endpoint)]
        ab.single.D2D28$endpoint[grep("less than",ab.single.D2D28$endpoint)] <- "0"
        ab.single.D2D28$endpoint <- log2(as.numeric(ab.single.D2D28$endpoint)+1)
        ab.single.D2D28$endpoint.rank <- rank(as.numeric(ab.single.D2D28$endpoint),na.last = "keep")/length(ab.single.D2D28$endpoint)
        
        # skip populations with small variance
        pop.to.test <- names(which(apply(ab.single.D2D28[grep("^C",colnames(ab.single.D2D28))],2,function(x){length(unique(x[!is.na(x)]))}) >= 5))
        for (pop in pop.to.test) {
          # draw scatter plot
          pop.ab.scatter.plots[[paste0(pop,"/",ab,"_",i)]] <- ggplot(ab.single.D2D28,aes_string(pop,"endpoint")) 
          
          #scale
          model.string <- paste0("endpoint ~ scale(",pop,")  + Age.at.day.of.vaccine + Gender.at.Birth")
          lm.res <- lm(as.formula(model.string),ab.single.D2D28)
          lm.summary <- summary(lm.res)
          ...
          #binary
          ab.single.D2D28$binary <- ab.single.D2D28[pop] > 0
          ...
          #rank
          ab.single.D2D28$rank <- rank(ab.single.D2D28[pop],na.last = "keep")
...
for (vt in var.types) {
  ...
  show.pop <- unique(sig.ab.pops[[vt]]$term)
  ...
  for (denorm in unique(pop.ab.lm.res.filtered$type)) {
    ...
    print(ggplot(tmp,aes(timepoint,term.label)) + 
            ...
            ggtitle(paste0("Relative to ",gsub(".norm.freq"," baseline",denorm)),vt) + xlab("Timepoint") + ylab("Cluster"))
  }
  dev.off()
}
```
...
```{r PCA}
pop.ab.target <- subset(ab.4plex,dose.timepoint == "2.D28")
colnames(pop.ab.target)[2] <- "orig.timepoint"
pop.ab.target <- reshape2::melt(pop.ab.target,measure.vars = c("S2P","Nprotein","RBD"))
  ...
  for (day in days[[dose]]) {
    ...
    if (ncol(dose.pop.freq) > 0) {
      for (i in 1:ncol(dose.pop.freq)) {
        ab.single.D2D28 <- cbind(dose.pop.freq[,i,drop=F],metadata[rownames(dose.pop.freq),c("Age.at.day.of.vaccine","Gender.at.Birth")])
        ab.single.D2D28$PC1 <- ab.pc1[rownames(ab.single.D2D28)]
        #ab.single.D2D28 <- merge(ab.single.D2D28,a,by="row.names")
        model.string <- paste0("PC1 ~ scale(",colnames(ab.single.D2D28)[1],")  + Age.at.day.of.vaccine + Gender.at.Birth")
        lm.res <- lm(as.formula(model.string),ab.single.D2D28)
        lm.summary <- summary(lm.res)
...
sig.pop <- subset(pc1.lm.res,Pr...t.. <= 0.05 & type == "cluster size")$cluster
...
```{r correlation}
# correlation between C2 and C13 frequencies
c2.freq <- subset(freq.data.normalized,population == "C2" & dose.timepoint == "2.D07")
c13.freq <- subset(freq.data.normalized,population == "C13" & dose.timepoint == "1.D10")
merged.freq <- merge(c2.freq,c13.freq,by="subject_id",all=T,suf=c(".c2",".c13"))
cont.table <- table(merged.freq$d1.norm.freq.c13 > 0,merged.freq$d2.norm.freq.c2 > 0)
or <- fisher.test(cont.table)
sc <- cor.test(merged.freq$d1.norm.freq.c13,merged.freq$d2.norm.freq.c2,method="spearman")
ggplot(merged.freq,aes(d2.norm.freq.c2,d1.norm.freq.c13)) +
  geom_hline(yintercept = 0,linetype="dashed",color="gray60") + geom_vline(xintercept = 0,linetype="dashed",color="gray60") +
  geom_point(size=2,alpha=0.5,fill="darkred",pch=21) + 
  theme_bw() + theme(plot.title =element_text(size=10,hjust=.5),text=element_text(size=10),aspect.ratio = 1) +
  xlab("C2 MBC RBD+S1+ freq.\nat v2D7") + ylab("C13 PB RBD+S1+ freq.\nat v1D10") +
  ggtitle(paste0("rho: ",  sprintf("%.3f",sc$estimate),", p=",sprintf("%.2e",sc$p.value),"\n",
       "OR: ", sprintf("%.3f",or$estimate),", p=",sprintf("%.2e",or$p.value),"")) 
ggsave(file.path(output.folder,paste0("scatter.plot.c2.vs.c13.",variable.of.interest,".pdf")),width=2.5,height=3)

# correlation with antibody titers
sig.pops.to.plot <- as.data.frame(sig.ab.pops$scale)
...
pdf(file.path(output.folder,paste0("scatter.plot.sig.ab.pop.correlation.",variable.of.interest,".pdf")),width=2.5,height=2.8)
for (i in 1:nrow(sig.pops.to.plot)) {
  if (norm.factor == "pct_of_cluster_total") {
    g2 <- pop.ab.scatter.plots[[paste0(sig.pops.to.plot[i,"population"],"/",sig.pops.to.plot[i,"antibody"],"_",sig.pops.to.plot[i,"protein"])]] +
      geom_point(size=2,alpha=0.75,fill="lightblue",pch=21) +
      ggtitle(paste0("Effect size: ",sprintf("%.3f",sig.pops.to.plot[i,"Estimate"]),
                     "\nP=",sprintf("%.3f",sig.pops.to.plot[i,"Pr...t.."])," (FDR=",sprintf("%.3f",sig.pops.to.plot[i,"p.adj"]),")")) +
      theme_bw() + theme(plot.title =element_text(size=10,hjust=.5),text=element_text(size=10),aspect.ratio = 1) +
      ylab(paste0("log2 ",sig.pops.to.plot[i,"protein"]," ",sig.pops.to.plot[i,"antibody"],"")) + 
      xlab(paste0(sig.pops.to.plot[i,"cluster.label"]," RBD+S1+ freq.\nat v",sig.pops.to.plot[i,"timepoint"]))
  } else {
    g2 <- pop.ab.scatter.plots[[paste0(sig.pops.to.plot[i,"population"],"/",sig.pops.to.plot[i,"antibody"],"_",sig.pops.to.plot[i,"protein"])]] + 
      geom_point(size=2,alpha=0.75,fill="orange",pch=21) +
      ggtitle(paste0("Effect size: ",sprintf("%.3f",sig.pops.to.plot[i,"Estimate"]),
                     "\nP=",sprintf("%.3f",sig.pops.to.plot[i,"Pr...t.."])," (FDR=",sprintf("%.3f",sig.pops.to.plot[i,"p.adj"]),")")) +
      theme_bw() + theme(plot.title =element_text(size=10,hjust=.5),text=element_text(size=10),aspect.ratio = 1) +
      ylab(paste0("log2 ",sig.pops.to.plot[i,"protein"]," ",sig.pops.to.plot[i,"antibody"],"")) +
